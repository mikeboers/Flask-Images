...
class Images(object):
    ...
    def init_app(self, app):
        ...
        app.add_url_rule(
            app.config['IMAGES_URL'] + '/<path:path>',
            app.config['IMAGES_NAME'],
            self.handle_request
        )
    ...
    def build_url(self, local_path, **kwargs):
        ...
        keys = 'background mode width height quality format padding'.split()
        for key in keys:
            if key in kwargs:
    ...
    def handle_request(self, path):
        ...
        if remote_url:
            ...
            path = os.path.join(
                ...
                hashlib.md5(
                    remote_url
                ).hexdigest() + os.path.splitext(remote_url)[1]
            )
        ...
        format = query.get('f', '').lower() or \
            os.path.splitext(path)[1][1:] or 'jpeg'
        ...
        return send_file(cache_path,
            ...
            cache_timeout=31536000 if has_version
                else current_app.config['IMAGES_MAX_AGE'],
        )
